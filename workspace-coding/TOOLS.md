...
```bash
python3 /usr/local/bin/image_search.py "关键词" --limit N --output /root/.openclaw/workspace/images --json
```
...
- 系统 `python3` 已指向 Python 3.12.7（通过 update-alternatives 配置）
- 所有 Python 脚本直接用 `python3` 运行即可
- 如果遇到 Python 版本错误，用 `python3 --version` 确认版本
